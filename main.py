...
SID = input("SID: ")
SID = "tr1ofvg0fmjrrsanm8nref8jda"
...
timer = time()
while not (game_tag := BeautifulSoup((r := sen.get(URL)).content, features="html.parser").select_one("#gameList input")):
    print(f"\r尚未開賣 {r.status_code} {format(time() - timer, '.2f')}s", end="")
...
while True:
    ...
    for _ in range(3):
        out = []
        for _ in range(3):
            img = imdecode(frombuffer(sen.get("https://tixcraft.com/ticket/captcha").content, uint8), IMREAD_COLOR)
            img = cvtColor(img, COLOR_BGR2GRAY)
            img = resize(img, (75, 40), interpolation=INTER_NEAREST)
